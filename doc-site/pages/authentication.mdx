...
Plaid uses the OAuth2 authorization code flow to acquire authorization from the end user, so Plaid can access your Core Exchange API on their behalf. Plaid redirects the end user to a web page your organization hosts for all authentication steps. At the end of the authorization code flow, Plaid receives an access token which both identifies the user and authenticates Plaid's access to your organization's Core Exchange API.
...
```curl
CURL request GET 'https://auth.firstplatypusbank.com/.well-known/openid-configuration'
```
...
Once the user has successfully completed all required authentication steps, your organization generates a temporary authorization code and redirects the user's browser back to the `redirect_uri`. Your organization includes the following query parameters with the request.
...
There are a number of different [client authentication schemes](https://openid.net/specs/openid-connect-core-1_0.html#ClientAuthentication) your server may support. Each scheme defines how Plaid will pass the client id and secret in the request. You may specify which methods your server supports via the `token_endpoint_auth_methods_supported` field in your openid-configuration object. If omitted, Plaid defaults to `client_secret_basic` as described in the [OAuth spec](https://www.rfc-editor.org/rfc/rfc6749#section-2.3.://www.rfc-editor.org/rfc/rfc6749#section-2.3.1).
...
At the completion of this step, Plaid now has everything it needs to access your Core Exchange API.
